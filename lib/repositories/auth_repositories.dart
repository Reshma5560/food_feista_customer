...
//get all address list api
  ...
  Future<dynamic> updateAddressApiCall({RxBool? isLoader, dynamic params, String? id}) async {
    try {
      isLoader?.value = true;
      await APIFunction().postApiCall(apiName: "${ApiUrls.updateAddressUrl}/$id", params: params).then(
        (response) async {
  ...
  Future<dynamic> signUpApi({dynamic params, RxBool? isLoader}) async {
    try {
      isLoader?.value = true;
      printData(key: "signUp params", value: params);
      await APIFunction().postApiCall(apiName: ApiUrls.registerUrl, params: params).then(
        (response) async {
          if (!isValEmpty(response) && response["status"] == true) {
            if (!isValEmpty(response["message"])) {
              toast(response["message"].toString());
